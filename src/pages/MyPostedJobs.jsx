...
const MyPostedJobs = () => {
  ...
  useEffect(() => {
    axios(`${import.meta.env.VITE_server_Url}/jobs/${user?.email}`)
      .then(res => {
        setJobs(res.data)
      })
    ...
    axios.delete(`${import.meta.env.VITE_server_Url}/job/${id}`)
      .then(res => {
        ...
        if (res.data.deletedCount > 0) {
          toast.success('Your job has been deleted.')
                    ...
                    jobs.map((job, i) => <tr key={job._id}>
                      ...
                      <td className='px-4 py-4 text-sm whitespace-nowrap'>
                        <div className='flex items-center gap-x-2'>
                          <p
                            className={`px-3 py-1 rounded-full ${job.category === 'Web Development' && 'text-blue-500'} ${job.category === 'Graphics Design' && 'text-green-500'} ${job.category === 'Digital Marketing' && 'text-pink-500'}  bg-blue-100/60
                              text-xs`}
                          >
                            {job.category}
                          </p>
