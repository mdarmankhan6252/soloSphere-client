...
const AllJobs = () => {
  ...
  const { data: jobs, error, isPending } = useQuery({
    queryKey: ['AllJobs'],
    queryFn: () => fetch(`${import.meta.env.VITE_server_Url}/jobs`)
      .then(res => {
        return res.json()
      })
  })
  ...
  if(isPending){
    return <p className="pt-[30%] text-center font-semibold text-2xl text-green-500">Loading...</p>
  }
